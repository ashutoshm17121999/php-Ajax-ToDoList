...
$(document).ready(function(){
    $(document).on('click','.edit',function(){
        ...
        $.ajax({
            ...
            // displayTodo(d);
            text1 = "";
            text1+='<input id="new-task" type="text"><button id="addBtn" name="addBtn">Add</button><button id="updateBtn" data-pid='+index+' name="updateBtn">Update</button>'
            $('#para').html(text1);
...
function displayTodo(myData){
    ...
    for (i=0;i<myData.length;i++){
        text += '<li><input type="checkbox" data-check='+i+' id="check" name="check"><label>'+myData[i]+'</label><input type="text"><button id="edit" data-ind='+i+' name="editBtn" class="edit">Edit</button><button id="delete" data-index='+i+' class="delete" name="deleteBtn">Delete</button></li>'
    }
